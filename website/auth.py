...
def reset_password():
    if request.method == 'POST':
        ...
        try:
            ...
            # Vulnerable SQL query - user input is directly concatenated into the query string
            db._execute_query(f"UPDATE employees SET password = '{hashed_password}' WHERE email = '{email}'")
...
def changepass():
    if request.method == 'POST':
        ...
        try:
            ...
            # Vulnerable SQL query - user input is directly concatenated into the query string
            db.cursor.execute(f"SELECT password FROM employees WHERE email = '{email}'")
