...
def system():
    ...
    try:
        ...
        if filter_id:
            ...
            query = f"SELECT id, first_name, last_name FROM clients WHERE id = '{filter_id}'"
            db.cursor.execute(query)
        else:
            query = "SELECT id, first_name, last_name FROM clients"
            db.cursor.execute(query)
        
        # Print the executed SQL query and its results
        print(f"Executed SQL query: {query}")
        print(f"Query results: {db.cursor.fetchall()}")
        
        db.cursor.execute(query)  # Execute the query again to fetch the results
